...
1. **Estrictamente Local (Offline & Local-Only):** La suite de aplicaciones funciona sin conexión a internet. Todos los datos, progresos y perfiles de los usuarios deben almacenarse exclusivamente en `localStorage`.
2. **Sin Dependencias Externas:** No se permiten bases de datos externas, llamadas a APIs, servidores backend, dependencias de `npm` (excepto para testing local como Playwright) ni inyecciones de contenido dinámico de terceros. Todo debe construirse con HTML, CSS y Vanilla JavaScript.
