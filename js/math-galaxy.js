...
function genCadet() {
  ...
  const types = ['count', 'add', 'moon_count', 'star_sub', 'planet_count', 'rocket_sub', 'astronaut_count', 'star_add', 'galaxy_count', 'alien_count', 'satellite_add', 'meteor_add', 'meteor_count', 'ufo_count', 'telescope_add', 'sun_count', 'sun_add', 'rocket_count', 'satellite_count', 'asteroid_add', 'planet_sub', 'sun_sub', 'meteor_pattern', 'cadet_new1', 'cadet_new2'];
  const type = types[rand(0, types.length - 1)];
  // PRUNED [2026-03-01]: Removed sun_count_2, planet_compare to stay within MAX limit
  if (type === 'cadet_new1') { const a = rand(1,4), b = rand(1,4); return { label: 'Astronauts', text: `👨‍🚀 ${a} + 🤖 ${b} =`, hint: 'Count them', answer: a + b, options: generateDistractors(a + b, 3, 2, 8), mode: 'choice' }; }
  if (type === 'cadet_new2') { const n = rand(2,6); return { label: 'Satellites', text: '🛰️'.repeat(n), hint: 'Count them', answer: n, options: generateDistractors(n, 3, 1, 8), mode: 'choice' }; }
...
function genExplorer() {
  ...
  const types = ['add', 'sub', 'planet_add', 'comet_compare2', 'double', 'space_compare', 'star_fraction', 'ufo_sub', 'meteor_sub', 'rocket_compare', 'astronaut_sub', 'robot_pattern', 'satellite_compare', 'comet_sub', 'alien_pattern', 'planet_compare', 'star_sub', 'planet_sub', 'comet_add', 'asteroid_sub', 'meteor_add_2', 'spaceship_add', 'satellite_add2', 'explorer_new1', 'explorer_new2'];
  const type = types[rand(0, types.length - 1)];
  // PRUNED [2026-03-01]: Removed alien_diff, asteroid_add_2 to stay within MAX limit
  if (type === 'explorer_new1') { const n = rand(5, 10); return { label: 'Fractions', text: `Half of ${n * 2} asteroids`, hint: 'Divide by 2', answer: n, options: generateDistractors(n, 3, 2, 15), mode: 'choice' }; }
  if (type === 'explorer_new2') { const a = rand(15, 25), b = rand(5, 14); return { label: 'Meteors', text: `There were ${a} meteors. ${b} fell. Left?`, hint: 'Subtract', answer: a - b, options: generateDistractors(a - b, 3, 5, 20), mode: 'choice' }; }
...
function genPilot() {
  ...
  const types = ['mult', 'div', 'frac_visual', 'ufo_mult', 'satellite_frac', 'comet_mult', 'asteroid_div', 'star_word', 'satellite_div', 'telescope_frac', 'planet_mult', 'ufo_div', 'astronaut_mult', 'rocket_div', 'moon_frac', 'alien_pct', 'asteroid_mult', 'alien_div', 'blackhole_mult', 'comet_frac', 'galaxy_div', 'nebula_frac', 'star_div', 'pilot_new1', 'pilot_new2'];
  const type = types[rand(0, types.length - 1)];
  // PRUNED [2026-03-01]: Removed moon_div, comet_mult_2 to stay within MAX limit
  if (type === 'pilot_new1') { const a = rand(4, 9), b = rand(4, 9); return { label: 'Mission', text: `${a} ships with ${b} crew. Total?`, hint: 'Multiply', answer: a * b, options: generateDistractors(a * b, 3, 10, 81), mode: 'choice' }; }
  if (type === 'pilot_new2') { const n = rand(2, 6); return { label: 'Fractions', text: `Divide ${n * 4} stars into 4 groups`, hint: 'Divide by 4', answer: n, options: generateDistractors(n, 3, 1, 10), mode: 'choice' }; }
...
function genCommander() {
  ...
  const types = ['big_add', 'big_mult', 'blackhole_div', 'galaxy_frac', 'percent', 'order_ops', 'galaxy_decimal', 'lightyear_percent', 'blackhole_ops', 'nebula_dec', 'blackhole_pct', 'galaxy_ops', 'comet_ops', 'star_decimal', 'asteroid_pct', 'blackhole_add', 'supernova_pct', 'gravity_ops', 'orbit_dec', 'quasar_ops', 'pulsar_dec', 'pulsar_add', 'galaxy_pct', 'commander_new1', 'commander_new2'];
  const type = types[rand(0, types.length - 1)];
  // PRUNED [2026-03-01]: Removed blackhole_sub, galaxy_pct_2 to stay within MAX limit
  if (type === 'commander_new1') { const a = [0.2, 0.5, 1.5, 2.5][rand(0, 3)], b = [10, 100, 1000][rand(0, 2)]; return { label: 'Decimals', text: `${a} × ${b} =`, hint: 'Move decimal', answer: a * b, options: generateDistractors(a * b, 3, (a*b)/10, (a*b)*10), mode: 'choice' }; }
  if (type === 'commander_new2') { const a = rand(2, 6), b = rand(2, 6), c = rand(1, 5); return { label: 'Order of Ops', text: `${a} + ${b} × ${c} =`, hint: 'Multiply first', answer: a + b * c, options: generateDistractors(a + b * c, 3, 5, 50), mode: 'choice' }; }
