...
function genCadet() {
  ...
  // PRUNED [2026-XX-XX]: Removed 'moon_count', 'star_sub' to make room for 'astronaut_add', 'satellite_sub' and stay within MAX 25 limit.
  ...
  if (type === 'astronaut_add') {
    ...
    return { label: 'Astronauts', text: `${a} 👨‍🚀 + ${b} 👨‍🚀 = ?`, hint: 'Count astronauts!', answer: a + b, options: generateDistractors(a + b, 3, 2, 10), mode: 'choice' };
  }
  if (type === 'satellite_sub') {
    const a = rand(3, 8), b = rand(1, a - 1);
    return { label: 'Satellites', text: `${a} 🛰️ - ${b} 🛰️ = ?`, hint: 'Subtract satellites', answer: a - b, options: generateDistractors(a - b, 3, 1, 7), mode: 'choice' };
  }
  if (type === 'alien_shape') {
...
function genExplorer() {
  ...
  // PRUNED [2026-XX-XX]: Removed 'alien_diff', 'asteroid_add_2' to make room for 'space_station_add', 'comet_pattern' and stay within MAX 25 limit.
  ...
  const types = ['add', 'sub', 'planet_add', 'comet_compare2', 'double', 'space_compare', 'star_fraction', 'ufo_sub', 'meteor_sub', 'rocket_compare', 'astronaut_sub', 'robot_pattern', 'satellite_compare', 'comet_sub', 'alien_pattern', 'planet_compare', 'star_sub', 'planet_sub', 'comet_add', 'asteroid_sub', 'meteor_add_2', 'spaceship_add', 'satellite_add2', 'space_station_add', 'comet_pattern'];
  const type = types[rand(0, types.length - 1)];
  if (type === 'space_station_add') { const a = rand(15,30), b = rand(5,14); return { label: 'Station Addition', text: `${a} 🛸 + ${b} 🛸 = ?`, hint: 'Add the stations', answer: a+b, options: generateDistractors(a+b, 3, 20, 50), mode: 'choice' }; }
  if (type === 'comet_pattern') { return { label: 'Comet Pattern', text: '☄️ 🌠 ☄️ 🌠 ?', hint: 'What comes next?', answer: '☄️', options: shuffle(['☄️', '🌠', '⭐', '🌕']), mode: 'choice' }; }
...
function genPilot() {
  ...
  // PRUNED [2026-XX-XX]: Removed 'moon_div', 'comet_mult_2' to make room for 'planet_frac', 'rocket_mult' and stay within MAX 25 limit.
  // PRUNED [2026-06-15]: Removed 'robot_mult', 'meteor_div' to make room for 'moon_div', 'comet_mult_2' and stay within MAX 25 limit.
  const types = ['mult', 'div', 'frac_visual', 'ufo_mult', 'satellite_frac', 'comet_mult', 'asteroid_div', 'star_word', 'satellite_div', 'telescope_frac', 'planet_mult', 'ufo_div', 'astronaut_mult', 'rocket_div', 'moon_frac', 'alien_pct', 'asteroid_mult', 'alien_div', 'blackhole_mult', 'comet_frac', 'galaxy_div', 'nebula_frac', 'star_div', 'planet_frac', 'rocket_mult'];
  const type = types[rand(0, types.length - 1)];
  if (type === 'planet_frac') { const n = rand(2, 6); return { label: 'Planet Fractions', text: `1/${n} of ${n * 3} 🪐`, hint: 'Divide by bottom number', answer: 3, options: generateDistractors(3, 3, 1, 10), mode: 'choice' }; }
  if (type === 'rocket_mult') { const a = rand(10, 15), b = rand(3, 6); return { label: 'Rocket Multiplication', text: `${a} 🚀 × ${b} = ?`, hint: 'Multiply', answer: a*b, options: generateDistractors(a*b, 3, 30, 90), mode: 'choice' }; }
...
function genCommander() {
  ...
  // PRUNED [2026-XX-XX]: Removed 'blackhole_sub', 'galaxy_pct_2' to make room for 'star_sci_not', 'nebula_ops' and stay within MAX 25 limit.
  // PRUNED [2026-06-15]: Removed 'robot_ops', 'ufo_decimal' to make room for 'blackhole_sub', 'galaxy_pct_2' and stay within MAX 25 limit.
  const types = ['big_add', 'big_mult', 'blackhole_div', 'galaxy_frac', 'percent', 'order_ops', 'galaxy_decimal', 'lightyear_percent', 'blackhole_ops', 'nebula_dec', 'blackhole_pct', 'galaxy_ops', 'comet_ops', 'star_decimal', 'asteroid_pct', 'blackhole_add', 'supernova_pct', 'gravity_ops', 'orbit_dec', 'quasar_ops', 'pulsar_dec', 'pulsar_add', 'galaxy_pct', 'star_sci_not', 'nebula_ops'];
  const type = types[rand(0, types.length - 1)];
  if (type === 'star_sci_not') { const n = rand(2,9); return { label: 'Scientific Notation', text: `${n} × 10³ ⭐`, hint: 'Add 3 zeros', answer: n * 1000, options: generateDistractors(n * 1000, 3, 100, 9000), mode: 'choice' }; }
  if (type === 'nebula_ops') { const a = rand(2,5), b = rand(3,7), c = rand(1,4); return { label: 'Order of Ops', text: `${a} + ${b} × ${c} = ?`, hint: 'Multiply first', answer: a + (b * c), options: generateDistractors(a + (b * c), 3, 5, 40), mode: 'choice' }; }
