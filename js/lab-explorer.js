...
const LabExplorer = (() => {
  ...
  let astroState = {
    ...
    starsTapped: 0,
    constellations: [
      { name: 'Big Dipper', stars: [{x:0.25,y:0.375},{x:0.375,y:0.425},{x:0.5,y:0.45},{x:0.625,y:0.55},{x:0.675,y:0.7},{x:0.875,y:0.65},{x:0.8,y:0.5}] },
      { name: 'Cassiopeia', stars: [{x:0.25,y:0.5},{x:0.375,y:0.7},{x:0.55,y:0.55},{x:0.7,y:0.675},{x:0.875,y:0.45}] }
    ],
    // Moon
    day: 14,
    // Planets
  ...
  function _initConstellations() {
    ...
    canvas.onpointerdown = (e) => {
      ...
      const cx = targetStar.x * canvas.width;
      const cy = targetStar.y * canvas.height;
      if (targetStar && Math.hypot(cx - mx, cy - my) < 30) {
        ...
        if (astroState.starsTapped === currentC.stars.length) {
          ...
          setTimeout(() => {
            alert(`Fact: The ${currentC.name} is one of the most famous patterns in the night sky!`);
  ...
  function _drawConstellations() {
    ...
    currentC.stars.forEach((s, i) => {
      ...
      const cy = s.y * canvas.height;
      ctx.beginPath();
      ctx.arc(cx, cy, i < astroState.starsTapped ? 6 : 4, 0, Math.PI*2);
      ...
      if (i === astroState.starsTapped) {
        ctx.beginPath();
        ctx.arc(cx, cy, 10 + Math.sin(Date.now()/200)*2, 0, Math.PI*2);
  ...
  function _initMoonPhases() {
    ...
    document.getElementById('moon-slider').oninput = (e) => {
      ...
      if (astroState.day === 0 || astroState.day === 14 || astroState.day === 28) {
         const phaseId = 'moon_star_' + astroState.day;
         if (!astroState.discovered.has(phaseId)) {
             astroState.discovered.add(phaseId);
             stars++;
             document.getElementById('exp-stars').textContent = `⭐ ${stars}`;
             _showFeedback('🌕');
             if (typeof playSound === 'function') playSound('correct');
             ...
             if (astroState.discovered.size >= 2) {
                 document.getElementById('next-btn').style.display = 'block';
                 _saveProgress();
             }
         }
  ...
  function _initPlanets() {
    ...
    const selectPlanet = (id) => {
        ...
        // Track discovered
        const planetDiscoveredId = id + '_discovered';
        if (!astroState.discovered.has(planetDiscoveredId)) {
            astroState.discovered.add(planetDiscoveredId);
            stars++;
            document.getElementById('exp-stars').textContent = `⭐ ${stars}`;
            _showFeedback('🪐');
            if (typeof playSound === 'function') playSound('correct');
            ...
            // Check if all three specific planets are discovered
            const discoveredCount = ['moon_discovered', 'mars_discovered', 'jupiter_discovered'].filter(pid => astroState.discovered.has(pid)).length;
            if (discoveredCount >= 3) { // Need to view all 3
                document.getElementById('next-btn').style.display = 'block';
                _saveProgress();
            }
        }
